...
class bot_database():
    ...
    def __init__(self):
        self.__engine__ = create_engine('sqlite:///bot.db', echo=False)
...
class bot_users_answers(Base):
    ...
    #bot_words = relationship("bot_words", back_populates="bot_examples")
    def __repr__(self):
        return "<Examples(id={}, word={}, sentence={})>".format(self.id,self.id_word,
                                                               self.sentence)

botdb = bot_database()

botdb.session.query(bot_user).all()
for inst in botdb.session.query(bot_words).all():
    print(inst)
